# 👋 Hello, I'm Prashant Gupta

🚀 DevOps & Cloud Enthusiast | ☁️ AWS Learner | 💻 Python Developer

---

## 🧑‍💻 About Me

* 🎓 B.Tech CSE (3rd Year) | Minor in Cloud Computing
* ☁️ Learning & working on **AWS, Docker, Linux, CI/CD**
* 🔧 Hands-on with **real-world projects & deployments**
* 🧠 Exploring **Automation, System Design & AI-based solutions**
* ⚡ Focused on building skills that companies actually need

---

## ⚙️ DevOps & Tech Stack

### 🐧 Core Skills

![Linux](https://img.shields.io/badge/Linux-black?logo=linux)
![Git](https://img.shields.io/badge/Git-orange?logo=git)
![GitHub](https://img.shields.io/badge/GitHub-black?logo=github)

### 🚀 DevOps Tools

![Docker](https://img.shields.io/badge/Docker-blue?logo=docker)
![CI/CD](https://img.shields.io/badge/CI/CD-green)
![AWS](https://img.shields.io/badge/AWS-yellow?logo=amazonaws)

### 💻 Programming

![Python](https://img.shields.io/badge/Python-green?logo=python)
![C++](https://img.shields.io/badge/C++-blue?logo=cplusplus)
![SQL](https://img.shields.io/badge/SQL-lightgrey)

### 🌐 Web

![HTML](https://img.shields.io/badge/HTML-orange)
![CSS](https://img.shields.io/badge/CSS-blue)
![JavaScript](https://img.shields.io/badge/JavaScript-yellow)

---

## 🚀 Featured Projects

### 🤖 AI & Python Projects

* 🤖 **AI Travel Planner**
* 🎣 **Phishing Awareness & Detection Tool**
* 😊 **Flask Mood Tracker**

---

### 🌐 Web & Full Stack Projects

* 🌐 **Portfolio Website**
* 📊 **Student Result Calculator**
* 🎓 **Django Angular Student Management System**

---

### ☁️ Cloud & System Projects

* ☁️ **Online Assignment Portal (AWS Architecture)**
* ⚙️ **Real-Time Process Monitoring System**

---

## 📊 GitHub Stats

![Stats](https://github-readme-stats.vercel.app/api?username=prashant-prashant\&show_icons=true\&theme=tokyonight)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=prashant-prashant\&layout=compact\&theme=tokyonight)

---

## 🔥 GitHub Streak

![Streak](https://streak-stats.demolab.com/?user=prashant-prashant\&theme=tokyonight)

---

## 🧠 Current Focus

* DevOps Projects (CI/CD Pipelines, Docker Deployment)
* AWS Hands-on (EC2, S3, IAM, VPC)
* Python Automation Scripts
* Making production-ready projects

---

## 📫 Connect with Me

* 💼 LinkedIn: https://www.linkedin.com/in/-gupta-prashant/
* 📧 Email: prashantgupta7616@gmial.com




